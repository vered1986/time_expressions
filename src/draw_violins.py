...
def main():
    ...
    for model in ["extractive", "lm_based"]:
        file = f"output/{model}/{args.lang}_24.json"
...
def draw_violin(grounding, labels, start_end_times, start_end_in_xaxis=False):
    ...
    sns.move_legend(ax, "best", ncol=2, title=None)
    ax.set(xlabel=None, ylabel=None)

    # Set the times
    times = range(1, max(37, max(d["Time"])))
    ax.set_yticks(range(1, max(37, max(d["Time"])), 2))
    ax.set_ylim([-1, len(times) + 1])
    ...
    for model_i, curr_start_end_times in enumerate(start_end_times):
        for exp_i, exp in enumerate(labels):
            ...
            ax.scatter(base_x + width * (2.38 * exp_i + model_i) - 0.16, start,
                       color=colors[model_i], edgecolor="black", marker="^")
            ax.scatter(base_x + width * (2.38 * exp_i + model_i) - 0.16, end,
                       color=colors[model_i], edgecolor="black", marker="v")
